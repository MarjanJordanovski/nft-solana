...
const CandyMachine = ({ walletAddress }) => {
  ...
  const getCandyMachineState = async() =>{
    ...
    console.log({
      itemsAvailable, itemsRedeemed, itemsRemaining, goLiveDate, goLiveDateTimeString,
    });
  };
//////////////////
  ...
  const mintToken = async () => {
	try{
    const mint = web3.Keypair.generate(); //creating account for NFT

    const userTokenAccountAddress = (
      await getAtaForMint(mint.publicKey, walletAddress.publicKey)
    )[0];
		console.log("Here1");
    const userPayingAccountAddress = candyMachine.state.tokenMint //additional instructions that we need to pass
    ...
    try {
      ...
      return (
		...
		//console.log("Here31", candyMachine.program.provider.connection,candyMachine.program.provider.wallet ,instructions,cleanupInstructions);
      console.log(e);
    }
    return [];
}catch(error){
